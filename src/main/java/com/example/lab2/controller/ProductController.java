...
public class ProductController {
    ...
    public ResponseEntity<?> getProducts(@RequestParam Map<String, String> params) {
        if (params.containsKey("minprice"))
            return new ResponseEntity<>(productService.getProductsByPriceLessThan(Double.parseDouble(params.get("minprice"))), HttpStatus.OK);
        if (params.containsKey("cat") && params.containsKey("maxprice"))
            return new ResponseEntity<>(productService.getProductsByCategoryAndPriceGreaterThan(params.get("cat"), Double.parseDouble(params.get("maxprice"))), HttpStatus.OK);
        if (params.containsKey("name"))
            return new ResponseEntity<>(productService.getProductByNameContaining(params.get("name")), HttpStatus.OK);
    ...
    public ResponseEntity<?> getProduct(@PathVariable long id) {
        ...
        return optionalProduct.isPresent() ? new ResponseEntity<>(optionalProduct.get(), HttpStatus.OK) : new ResponseEntity<>(HttpStatus.NOT_FOUND);
    }
